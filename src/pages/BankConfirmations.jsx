  ...
  return (
    <div style={{ padding: '0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0369a1' }}>Bank Confirmations</h1>
      ...
      <div style={{ background: 'white', borderRadius: '1rem', border: '1px solid #e0f2fe', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {records.length === 0 ? <tr><td colSpan="5" style={{ padding: '2rem', textAlign: 'center', color: '#0369a1' }}>No offline data.</td></tr> :
              records.map((r, idx) => (
                <tr key={idx} style={{ borderBottom: '1px solid #f0f9ff' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <div style={{ display:'flex', gap:'0.5rem' }}>
                      <button onClick={() => { setForm({ ...r }); setIsModalOpen(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6' }}><Edit size={14} /></button>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', padding: '1.5rem', borderRadius: '1rem', width: '400px' }}>
            <h2>{form.id ? 'Edit' : 'Add'} Confirmation</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginTop: '1rem' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '0.6rem', background: '#0369a1', color: 'white', border: 'none', borderRadius: '0.5rem', fontWeight: 700 }}>Save Locally</button>
            </form>
          </div>
        </div>
      )}
