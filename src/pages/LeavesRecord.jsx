...
const LeavesRecord = () => {
  ...
  const [leaves, setLeaves] = useState([
    { id: '1', type: 'Sick Leave',   startDate: '2026-03-01', endDate: '2026-03-03', days: 3, reason: 'Fever',         status: 'Approved' },
    { id: '2', type: 'Casual Leave', startDate: '2026-03-10', endDate: '2026-03-10', days: 1, reason: 'Personal work', status: 'Pending'  },
  ]);
  ...
  const handleDelete = id => {
    if (window.confirm('Delete this leave record?')) setLeaves(leaves.filter(l => l.id !== id));
  };
  ...
  const handleSave = e => {
    e.preventDefault();
    if (formState.id) {
      setLeaves(leaves.map(l => l.id === formState.id ? { ...formState, status: l.status } : l));
    } else {
      setLeaves([...leaves, { ...formState, id: Date.now().toString(), status: 'Pending' }]);
    }
