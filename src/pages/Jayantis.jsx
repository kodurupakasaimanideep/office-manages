...
const Jayantis = () => {
  ...
  const handleSave = (e) => {
    ...
    if (form.id) { 
      setEvents(events.map(ev => ev.id === form.id ? { ...payload } : ev));
    } else { 
      setEvents([...events, { ...payload, id: Date.now().toString() }]);
    }
    setIsModalOpen(false);
  };

  const deleteRecord = (id) => { if (window.confirm('Delete?')) setEvents(events.filter(ev => ev.id !== id)); };
    ...
    <div style={{ padding: '0' }}>
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
          ...
          <h1 style={{ fontSize: '2.5rem', fontWeight: 900, margin: 0 }}>Celebrations (Offline)</h1>
        </div>
        <p style={{ opacity: 0.9, fontSize: '1.1rem', fontWeight: 500 }}>Secure offline tracking for cultural event expenditures.</p>
      ...
      <div className="glass-card" style={{ borderRadius: '1.5rem', overflow: 'hidden', border: `2px solid ${THEME.border}`, background: 'white' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {(events || []).length === 0 ? <tr><td colSpan="4" style={{ padding: '4rem', textAlign: 'center', color: '#92400e' }}>No offline data available.</td></tr> :
              (events || []).map((e, index) => (
                <tr key={index} style={{ borderBottom: `1px solid ${THEME.border}`, background: index % 2 === 0 ? 'white' : '#fffcf1' }}>
                  ...
                  <td style={{ padding: '1.5rem 2rem' }}>
                    <div style={{ display: 'flex', gap: '1rem' }}>
                      ...
                      <button onClick={() => deleteRecord(e.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }}><Trash2 size={20} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(69,26,3,0.3)', backdropFilter: 'blur(12px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="glass-card" style={{ padding: '2.5rem', borderRadius: '2rem', width: '500px', background: 'white', border: `2px solid ${THEME.border}` }}>
            <h2 style={{ marginBottom: '2rem', fontWeight: 900, color: THEME.primary }}>{form.id ? 'Edit' : 'Add'} Celebration</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '1rem', background: THEME.gradient, color: 'white', border: 'none', borderRadius: '1rem', fontWeight: 900 }}>Save Locally</button>
            </form>
          </div>
        </div>
      )}
