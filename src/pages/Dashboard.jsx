...
/* ─── Dashboard ──────────────────────────────────────────────────────────── */
const Dashboard = () => {
  const rawFiles = useQuery(api.dashboardFiles.get) || [];
  const files = rawFiles.map(f => ({ ...f, id: f._id }));
  
  const addFile = useMutation(api.dashboardFiles.add);
  const updateFile = useMutation(api.dashboardFiles.update);
  const removeFile = useMutation(api.dashboardFiles.remove);
  ...
  const processFiles = async rawFiles => {
    ...
    for (const file of rawFiles) {
      ...
      reader.onload = async e => {
        await addFile({
          idLocal: Date.now().toString() + Math.random().toString(36).slice(2),
  ...
  const toggleImportant = async id => {
    ...
    if(file) await updateFile({ id: file._id, important: !file.important });
  };
  const deleteFile      = async id => { 
    if (window.confirm('Delete this file?')) {
      const file = files.find(f => f.id === id);
      if(file) await removeFile({ id: file._id });
    }
  };
