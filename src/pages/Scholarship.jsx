...
const Scholarship = () => {
  const [obcRecords, setObcRecords] = useState(() => {
    try { return JSON.parse(localStorage.getItem('scholarship_obc') || '[]'); } catch { return []; }
  });
  const [bcRecords, setBcRecords] = useState(() => {
    try { return JSON.parse(localStorage.getItem('scholarship_bc') || '[]'); } catch { return []; }
  });
  const [modal, setModal] = useState({ open: false, section: null });
  const [form, setForm] = useState(defaultForm());

  useEffect(() => { localStorage.setItem('scholarship_obc', JSON.stringify(obcRecords)); }, [obcRecords]);
  useEffect(() => { localStorage.setItem('scholarship_bc', JSON.stringify(bcRecords)); }, [bcRecords]);
  ...
  const handleSave = (e) => {
    e.preventDefault();
    const setFn = modal.section === 'OBC' ? setObcRecords : setBcRecords;
    if (form._id) {
      setFn(prev => prev.map(r => r._id === form._id ? { ...form } : r));
    } else {
      setFn(prev => [...prev, { ...form, _id: Date.now().toString() }]);
    }
    closeModal();
  };
  ...
  const handleDelete = (section, id) => {
    if (window.confirm('Delete this record?')) {
      const setFn = section === 'OBC' ? setObcRecords : setBcRecords;
      setFn(prev => prev.filter(r => r._id !== id));
    }
  };
