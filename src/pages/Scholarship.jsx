...
function ScholarshipSection({ title, type, color, records, onAdd, onEdit, onDelete }) {
  ...
  return (
    <div className="glass-card" style={{ borderRadius:'1.5rem', overflow:'hidden', border:`2px solid ${color.border}`, marginBottom:'2.5rem', background: 'white' }}>
      <div style={{ background:color.header, padding:'1.5rem 2rem', display:'flex', justifyContent:'space-between', alignItems:'center', color: 'white' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:'0.85rem', opacity:0.85, fontWeight:600 }}>{completedCount} Completed Records</div>
          </div>
        </div>
      ...
      <div style={{ overflowX:'auto' }}>
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          ...
          <tbody>
            {(records || []).length === 0 ? <tr><td colSpan="5" style={{ textAlign:'center', padding:'4rem', color:'var(--text-muted)' }}>No records discovered locally.</td></tr> : (
              (records || []).map((r, i) => (
...
const Scholarship = () => {
  const [records, setRecords] = useState(() => JSON.parse(localStorage.getItem('scholarships') || '[]'));
  useEffect(() => localStorage.setItem('scholarships', JSON.stringify(records)), [records]);
  ...
  const handleDelete = (id) => { if (window.confirm('Delete record?')) setRecords(records.filter(r => r.id !== id)); };

  return (
    <div style={{ padding: '0' }}>
      <div style={{ marginBottom: '2.5rem' }}>
        <h1 style={{ fontSize: '2.25rem', fontWeight: 900, color: '#1e293b' }}>Scholarship Analytics</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '1rem' }}>Local monitoring of scholarship disbursements.</p>
      </div>
      ...
      {modal.open && (
        <div style={{ position:'fixed', inset:0, background:'rgba(15, 23, 42, 0.4)', backdropFilter:'blur(10px)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
          <div className="glass-card" style={{ background:'white', padding:'2.5rem', borderRadius:'2rem', width:'480px', border: `2px solid ${SECTION_COLORS[modal.section].border}` }}>
            <h2 style={{ marginBottom:'1.5rem', fontWeight: 900, color: SECTION_COLORS[modal.section].dot }}>{form.id ? 'Edit' : 'Add'} Entry</h2>
            <form onSubmit={handleSave} style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
              ...
              <div style={{ display:'flex', gap:'1rem', marginTop:'1.5rem' }}>
                <button type="submit" style={{ flex:2, padding:'1rem', background:SECTION_COLORS[modal.section].header, color:'white', border:'none', borderRadius:'1rem', cursor:'pointer', fontWeight:900 }}>Save Changes</button>
