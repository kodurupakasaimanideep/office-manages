...
const Schemes = () => {
  ...
  const updateScheme = useMutation(api.schemes.update);
  const removeScheme = useMutation(api.schemes.remove);
  ...
  const handleSave = async (e) => {
    ...
    if (form.id) { await updateScheme({ ...payload, id: form.id }); } 
    else { await addScheme({ ...payload }); }
    setIsModalOpen(false);
  };

  const deleteScheme = async (id) => { if (window.confirm('Delete?')) await removeScheme({ id }); };
  ...
  return (
    <div style={{ padding: '0' }}>
      <div style={{ 
        ...
        <h1 style={{ fontSize: '2.5rem', fontWeight: 900, margin: 0 }}>Government Schemes (Cloud Sync)</h1>
        <p style={{ opacity: 0.9, fontSize: '1.1rem', fontWeight: 500 }}>Global monitoring of welfare programs.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
        {records.length === 0 ? (
          <div style={{ textAlign:'center', padding:'4rem', background:'white', borderRadius:'2rem', border:`2.2px dashed #c7d2fe`, gridColumn: '1/-1' }}>
            <h3 style={{ color: '#6366f1', fontWeight: 800 }}>No Cloud Schemes</h3>
            <button onClick={() => { setForm({ id: null, name: '', headOfSection: '', status: 'Pending', progress: 0 }); setIsModalOpen(true); }} style={{ padding: '0.8rem 2rem', background: '#6366f1', color: 'white', border: 'none', borderRadius: '1rem', fontWeight: 900, cursor: 'pointer', marginTop: '1rem' }}>+ Create Cloud Scheme</button>
          </div>
        ) : (
          records.map((r, i) => (
            <div key={i} className="glass-card" style={{ padding: '2rem', borderRadius: '2rem', border: `2px solid #c7d2fe`, background: 'white' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => { setForm({ ...r, id: r._id }); setIsModalOpen(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6' }}><Edit size={16} /></button>
                  <button onClick={() => deleteScheme(r._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }}><Trash2 size={16} /></button>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(30,27,75,0.4)', backdropFilter: 'blur(12px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="glass-card" style={{ padding: '2.5rem', borderRadius: '2rem', width: '500px', background: 'white', border: `3px solid #c7d2fe` }}>
            <h2 style={{ marginBottom: '2rem', fontWeight: 900, color: '#6366f1' }}>Cloud {form.id ? 'Edit' : 'New'} Scheme</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '1.1rem', background: '#6366f1', color: 'white', border: 'none', borderRadius: '1.5rem', cursor: 'pointer', fontWeight: 900 }}>Save to Cloud</button>
            </form>
          </div>
        </div>
      )}
