...
const RiceIndent = () => {
  ...
  const removeIndent = useMutation(api.riceIndents.remove);
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (form.id) { await updateIndent({ ...form, id: form.id }); } 
    else { await addIndent({ ...form }); }
    setIsModalOpen(false);
  };

  const deleteIndent = async (id) => { if (window.confirm('Delete?')) await removeIndent({ id }); };

  return (
    <div style={{ padding: '0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#c8813a' }}>Rice Indents (Cloud Sync)</h1>
      ...
      <div style={{ background: 'white', borderRadius: '1rem', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {indents.length === 0 ? <tr><td colSpan="5" style={{ padding: '2rem', textAlign: 'center', color: '#92400e' }}>No cloud data.</td></tr> :
              indents.map((i, idx) => (
                <tr key={idx} style={{ borderBottom: '1px solid #fffbeb' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <button onClick={() => { setForm({ ...i, id: i._id }); setIsModalOpen(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6', marginRight: '0.5rem' }}><Edit size={14} /></button>
                    <button onClick={() => deleteIndent(i._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }}><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
          </tbody>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', padding: '1.5rem', borderRadius: '1rem', width: '400px' }}>
            <h2>Cloud {form.id ? 'Edit' : 'Add'} Indent</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginTop: '1rem' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '0.6rem', background: '#c8813a', color: 'white', border: 'none', borderRadius: '0.5rem', fontWeight: 700 }}>Save to Cloud</button>
            </form>
          </div>
        </div>
      )}
