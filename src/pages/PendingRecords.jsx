...
const PendingRecords = () => {
  ...
  const scholarships = useQuery(api.scholarships.get) || [];
  const scholarshipCompleted = scholarships.filter(r => r.status === 'Completed').length;
  const scholarshipPending   = scholarships.filter(r => r.status === 'Pending').length;
  const scholarshipTotalItems     = scholarships.reduce((sum, r) => sum + Number(r.total || 0), 0);
  const scholarshipCompletedItems = scholarships.reduce((sum, r) => sum + Number(r.completed || 0), 0);
  const scholarshipPct            = scholarshipTotalItems === 0 ? 0 : Math.round((scholarshipCompletedItems / scholarshipTotalItems) * 100);
  ...
  const riceCompleted = riceIndents.filter(r => r.status === 'Approved').length;
  const ricePending = riceIndents.filter(r => r.status === 'Pending').length;
  const schemesCompleted = schemes.filter(r => r.status === 'Completed').length;
  const schemesPending = schemes.filter(r => r.status === 'Pending').length;

  const officeTotal         = total + riceIndents.length + scholarshipTotalItems + schemes.length;
  const officeCompleted     = completedCount + riceCompleted + scholarshipCompletedItems + schemesCompleted;
  const officePending       = pendingCount + ricePending + (scholarshipTotalItems - scholarshipCompletedItems) + schemesPending;
  ...
  const categories = [
    ...
    { 
      label: 'Rice Indent', 
      completed: riceCompleted, 
      pendingCount: ricePending, 
      total: riceIndents.length, 
      ...CATEGORY_COLORS[1] 
    },
    { 
      label: 'Scholarship', 
      completed: scholarshipCompletedItems, 
      pendingCount: scholarshipTotalItems - scholarshipCompletedItems, 
      total: scholarshipTotalItems, 
      isNumeric: true, 
      customPct: scholarshipPct, 
      ...CATEGORY_COLORS[2] 
    },
    { 
      label: 'Schemes', 
      completed: schemesCompleted, 
      pendingCount: schemesPending, 
